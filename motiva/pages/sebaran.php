...
<html>
<head>
<title>WebGIS Sebaran Cagar Budaya</title>
...
<input type="checkbox" id="layerCagar" checked> Cagar Budaya
</div>
...
<div class="stat-box">
<b>Total Cagar:</b> <span id="totalCagar">0</span>
</div>
...
// GLOBAL
let layerCagar, layerKecamatan, layerSungai;
let cagarData = [];
let chart;
...
// ================= POPUP =================
function popupCagar(p){

    const img = p.foto
        ? `<img src="../uploads/foto/${p.foto}" style="width:100%;border-radius:8px;margin-top:8px;">`
    ...
    return `
        <div style="min-width:250px">
            ...
            <table style="font-size:13px">
                ...
                <tr><td><b>Sumber</b></td><td>: ${p.sumber}</td></tr>
            </table>
            ${img}
        </div>
    `;
...
function renderCagar(data){
    ...
    layerCagar = L.geoJSON({
        ...
        pointToLayer:(f,latlng)=>L.circleMarker(latlng,{
            radius:6, fillColor:"#e11d48", color:"#fff", fillOpacity:0.9
        }),
        onEachFeature:(f,l)=>l.bindPopup(popupCagar(f.properties))
    }).addTo(map);
}
...
.then(data=>{
    ...
    layerKecamatan = L.geoJSON(data,{
        ...
        style:f=>{
            ...
            let jml = countCagar(nama);
            return { fillColor:getColor(jml), weight:1.5, color:'#333', fillOpacity:0.6 };
        },
        ...
        onEachFeature:(f,l)=>{
            ...
            l.bindPopup(`<b>${nama}</b><br>Total Cagar: ${jml}`);

            l.on('click',()=>{
                let list = cagarData.filter(x =>
                    (x.properties.kecamatan||'').toLowerCase() === nama.toLowerCase()
                );
                showList(nama,list);
            });
        }
...
// ================= COUNT =================
function countCagar(kec){
    return cagarData.filter(f =>
        (f.properties.kecamatan||'').toLowerCase() === kec.toLowerCase()
    ).length;
}

// ================= LIST =================
function showList(nama,list){

    let html = `<b>${nama}</b><br>Total: ${list.length}<hr>`;

    list.forEach(item=>{
        html += `<div>📍 ${item.properties.nama}</div>`;
    });

    document.getElementById("listCagar").innerHTML = html;
}
...
document.getElementById("searchKecamatan").onchange = function(){
    ...
    let filtered = cagarData.filter(f =>
        (f.properties.kecamatan||'').toLowerCase().includes(kec)
    );

    renderCagar(filtered);
    updateDashboard(filtered);
    showList(this.value, filtered);
};

// ================= DASHBOARD =================
function updateDashboard(data){

    document.getElementById("totalCagar").innerText = data.length;
...
function renderChart(data){
    ...
    chart = new Chart(ctx,{
        ...
        data:{
            ...
            datasets:[{label:'Jumlah Cagar',data:Object.values(data)}]
        }
    });
}
